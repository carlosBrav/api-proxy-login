...
function validateLimitsArray(param, name) {
    if (!Array.isArray(param))
        throw createErrorResponse(400, `El parámetro '${name}' no es válido.`);

    const allowedLimits = ["DAILY_DEPOSIT", "MONTHLY_DEPOSIT", "WEEKLY_DEPOSIT"];

    param.forEach((item) => {
        if (typeof item !== "object" || item === null)
            throw createErrorResponse(400, `El parámetro '${name}' no es válido.`);

        if (!allowedLimits.includes(item.limit))
            throw createErrorResponse(400, `El parámetro '${name}' no es válido.`);
...
export const lambda_handler = async (event, context, callback) => {
    ...
    try {
        ...
        const limits = validateLimitsArray(payload.limits, "limits");
        ...
        const { data } = await callCalimacoApi(`${CALIMACO_BASE_URL}/data/saveUserDepositLimits`, {
            "company": "ACP",
        ...
        if (data.code == -10)
            return callback(null, createErrorResponse(401, "Los limites ingresado no son validos"));

        if (data.code == -25)
            return callback(null, createErrorResponse(400, "Restricción de Regulación: 24 horas"));

        if (data.result == "error") {
            console.error("API saveUserDepositLimits:", data);
