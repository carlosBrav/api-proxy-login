...
export const lambda_handler = async (event, context, callback) => {
  ...
  try {
    ...
    const init = validateNumberParam(queryParams.init, "init") ?? 0;
    const end = validateNumberParam(queryParams.end, "end") ?? 20;
    ...
    const { status, data } = await callCalimacoApi(
      `${CALIMACO_BASE_URL}/data/getUserActivePromotions`,
      {
        company: "ACP",
    ...
    //   console.error("API getUserActivePromotions:", status);
    //   return callback(
    //     null,
    //     createErrorResponse(400, "El código ingresado no es válido")
    //   );
    // }
