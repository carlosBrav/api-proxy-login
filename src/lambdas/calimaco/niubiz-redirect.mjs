...
export const lambda_handler = async (event, context, callback) => {
    ...
    try {
        ...
        const consent = isValidString(payload.consent, "consent");
        const agree = isValidBoolean(payload.agree, "agree");
        ...
        const { data } = await callCalimacoApi(`${CALIMACO_BASE_URL}/data/saveUserConsent`, {
            "company": "ACP",
        ...
        if (data.result == "error") {
            console.error("API autoexclude:", data);
