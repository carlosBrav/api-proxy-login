...
export const lambda_handler = async (event, context, callback) => {
  ...
  try {
    ...
    const provider = isValidString(payload.provider, "proveedor");
    ...
    const { data } = await callCalimacoApi(
      `${CALIMACO_BASE_URL}/casino/getProviderInfo`,
      {
        company: "ACP",
    ...
    if (data.code == -2) {
      console.error("API getProviderInfo:", data);
      return callback(
        null,
        createErrorResponse(401, "Sin Autorización")
      );
    }
     ...
     if ( data.code == -33) {
      return callback(
        null,
        createErrorResponse(401, "Sin Autorización o Sessión expirada")
      );
    }

    if (data.code == -10) {
      console.error("API getProviderInfo:", data);
      return callback(null, createErrorResponse(404, "No éxiste el provedor"));
    }
