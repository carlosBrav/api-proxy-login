...
// No requiere campos en el body (solo el token)
...
const secureLog = (label, data) => {
    // 🟢 [SEGURIDAD]: Ocultamos datos sensibles
    const SENSITIVE_KEYS = ["aws-token", "authorization", "session", "session_col", "lobby", "methods"];
...
// 🟢 [COMPATIBILIDAD]: Form-UrlEncoded (Requerido por Legacy)
...
const validateRequest = (event) => {
    ...
    // D. Validar Campos Requeridos (Vacío)
...
export const lambda_handler = async (event) => {
    ...
    try {
        ...
        // PASO 4: Llamar API
        secureLog("Proceso", { step: "Llamando API Externa", url: `${CALIMACO_BASE_URL}/contents/getDepositLobby` });
        const apiResponse = await callExternalApi(`${CALIMACO_BASE_URL}/contents/getDepositLobby`, externalApiBody, userAgent);
        ...
        // Caso: Error genérico (incluye "Promotion already activated", etc.)
        if (apiResponse.result === "error" || (apiResponse.code && apiResponse.code !== 0)) {
            console.warn(`API Error: ${apiResponse.description || apiResponse.message}`);
            // Usamos la descripción dinámica de la API en lugar de hardcodear
            const msg = apiResponse.description || apiResponse.message || "Error al obtener lobby";
            return createResponse(400, { 
                message: msg,
