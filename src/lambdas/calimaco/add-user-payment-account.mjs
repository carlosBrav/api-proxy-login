...
function isValidJson(value, name) {
...
export const lambda_handler = async (event, context, callback) => {
    ...
    try {
        ...
        const payment_account = isValidJson(payload.payment_account, "payment_account");
        ...
        const { data } = await callCalimacoApi(`${CALIMACO_BASE_URL}/data/addUserPaymentAccount`, {
            "company": "ACP",
        ...
        if(data.code){
            console.error("API addUserPaymentAccount error:", data);

            if (data.code == -15)
                return callback(null, createErrorResponse(400, "Completar los campos obligatorios"));
    
            if (data.code == -10)
                return callback(null, createErrorResponse(400, "El payment_account ingresado no es válido"));
        }

        if (data.result == "error") {
            console.error("API addUserPaymentAccount:", data);
