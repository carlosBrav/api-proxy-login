...
function validateCodePromotion(code) {
    ...
    if (!code || Number.isNaN(num) || num <= 0 || (typeof code !== "number")) {
...
export const lambda_handler = async (event, context, callback) => {
    ...
    try {
        ...
        const { status, data } = await callCalimacoApi(`${CALIMACO_BASE_URL}/data/cancelUserPromotion`, {
            "company": "ACP",
        ...
        if (data.code == -2) {
            console.error("API cancelUserPromotion: ", data);
            return callback(null, createErrorResponse(401, "Sin Autorización"));
        }

        if (data.result == "error") {
            console.error("API cancelUserPromotion:", data);
            return callback(null, createErrorResponse(400, "El código ingresado no es válido"));
        }
