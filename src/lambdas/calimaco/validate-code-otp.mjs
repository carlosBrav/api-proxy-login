...
// Campos requeridos (incluye codigoOtp)
const REQUIRED_FIELDS = ["jugadorIdAlira", "codigoPais", "numeroTelefono", "codigoOtp"];
...
const secureLog = (label, data) => {
    const SENSITIVE_KEYS = ["aws-token", "authorization", "session", "session_col", "sessionIdAlira", "numeroTelefono", "codigoOtp", "email"];
...
const validateDataTypes = (payload) => {
    ...
    // Validar OTP
    const otpCode = String(payload.codigoOtp).trim();
    if (!/^\d{4,6}$/.test(otpCode)) errors.push("'codigoOtp' debe contener 4-6 dígitos");

    if (errors.length > 0) throw { statusCode: 400, message: errors.join(". ") };

    return { playerId, countryCode, cellPhone, otpCode };
};
...
export const lambda_handler = async (event) => {
    ...
    try {
        ...
        const { playerId, countryCode, cellPhone, otpCode } = validateDataTypes(payload);
        ...
        if (apiResponse.errorCodigo && apiResponse.errorCodigo != 0) {
            ...
            if (Number(errorCodigo) === -2) {
                console.warn("API Error Code -2: Sesión Expirada");
            ...
            return createResponse(400, {
                message: apiResponse.errorMensaje || "Error validando OTP",
